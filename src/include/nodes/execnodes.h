...
//#ifdef WIN_FUN_OPT
//	TupleTableSlot	*opt_exct_slot;	/* for the computation of arguments of the window functions */
//#endif
...
//#ifdef WIN_FUN_OPT
	List		*opt_args;		/* backup arguments for useful tuple */
//#endif
} WindowFuncExprState;
...
typedef struct WindowAggState
{
	...
	/* add by cywang, for timing */
	Instrumentation	*instr_total_part;			/* the time cost during a partition */
	Instrumentation	*instr_locateheadpos_part;		/* the time cost in locating to a position during a partition*/
	Instrumentation	*instr_recompute_part;	/* the time cost in recomputing for the previous tuples before current position during a partition */
	//Instrumentation	*instr_checkinframe_part;	/* the time cost in checking whether a tuple in the frame during a partition*/
	//Instrumentation	*instr_setmarkpos_part;		/* the time cost in WinSetMarkPosition() during a partition */
	Instrumentation	*instr_cpu_afterspooled;				/* the time of compute except instr_locateheadpos_part */
	Instrumentation	*instr_recompute_cpu;
	Instrumentation	*instr_io_afterspooled;

//#ifdef WIN_FUN_OPT
	int				opt_current_ptr;		/* the read pointer for partition read, like the current_ptr in WindowAggState*/

	//Expr			*opt_expr;				/* no need, be used to make an TargetEntry */
	//List			*opt_targetList;		/* no need, the target list transforming from a initial tuple to a useful tuple */
	TupleDesc		opt_tupdesc;			/* the tuple description of the useful tuple */
	//ProjectionInfo	*opt_projInfo;			/* no need, first idea is to project the initial tuple to a useful tuple, but finally found it can be done with TupleDesc and useful2init. */

	TupleTableSlot	*opt_first_part_slot;	/* seems no need, the first opt tuple slot of the current or next partition */
	TupleTableSlot	*opt_agg_row_slot;		/* temporary slots for useful tuples */
	TupleTableSlot	*opt_temp_slot_1;
	TupleTableSlot	*opt_temp_slot_2;
	TupleTableSlot	*buffer_slot;			/* used by the buffer */
	AttrNumber		*useful2init;			/* map the useful attributes to initial attributes, used to make a Var node, then TargetEntry=>targetList=>TupleDesc */
	AttrNumber		*init2useful;			/* no used, map the initial attributes to useful attributes, of cause only the useful attributes are set */
	int				opt_maxPos;				/* for attribute extraction optimization */
	TupleTableSlot	*opt_scantupslot;		/* used for computing peers of current row */

	int				framehead_ptr;
//#endif

	/* add by cywang, for reducing recompute */
	int			opt_tempTransValue_num;
	int			opt_active_tempTransValue;
} WindowAggState;
